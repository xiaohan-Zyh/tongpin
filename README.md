...
### 4. 接入 Redis 实现数据持久化（推荐）

Vercel 是多实例 Serverless 架构，实例会被回收，进程内存不保证跨请求保留。
本项目通过 `src/lib/kv.ts` 抽象层解决该问题：

- **已接入 Redis**：数据跨实例共享，观点、请求、对话不会丢失，
  OAuth state 防重放依赖 `SET NX` 原子命令，多实例下依然可靠。
- **未接入 Redis**：自动回退进程内存，本地开发与测试无需外部服务；
  线上则会出现数据跨实例不一致（详见第 5 节）。

#### 接入步骤

> Vercel KV 已于 2024 年底下线并迁移至 Upstash Redis，现从 Marketplace 接入。

1. 进入项目 → **Storage** 标签 → **Create Database**
2. 选择 **Upstash** 的 **Redis**（有免费额度，够黑客松 Demo 使用）
3. 创建时区域建议选离函数最近的（本项目函数区域为 `hkg1` 香港）
4. 创建完成后点 **Connect Project**，关联到本项目

Vercel 会自动注入连接凭据，**无需手动填写环境变量**。
代码同时兼容两套命名，任意一套存在即可生效：
...
5. 关联后**触发一次重新部署**，Redis 才会生效

#### 如何确认已生效

登录后发表一条观点，刷新页面若内容仍在，即说明持久化成功。
...
- 交流请求与对话可能时有时无
- OAuth state 防重放降级：跨实例时可能拦不住重放。
  签名校验、有效期校验、Cookie 一次性删除仍然有效，登录 CSRF 防护未失效

演示用的 8 条种子观点会在首次访问时写入，因此**匹配功能的演示效果始终正常**：
登录后发表观点即可看到相似度与匹配理由。
...
- **数据持久化**：已通过 `src/lib/kv.ts` 支持 Upstash Redis。接入后数据跨实例
  持久保存、state 防重放依赖 `SET NX` 原子命令可靠生效；未接入时自动回退进程内存，
  线上会出现跨实例数据不一致。接入步骤见「二、部署到 Vercel」第 4 节。
- **存储规模**：广场与匹配为全量读取后在内存中筛选排序，适合 Demo 数据量；
  数据量增大后需改为索引查询或引入向量检索。
